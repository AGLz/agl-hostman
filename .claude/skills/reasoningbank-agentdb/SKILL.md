...
description: "Implement ReasoningBank adaptive learning with AgentDB's high-performance backend delivering 150x-12,500x faster operations enabling trajectory tracking for decision paths, verdict judgment evaluating outcome success/failure, memory distillation condensing experiences into patterns, and pattern recognition identifying successful strategies with 100% backward compatibility migrating from legacy ReasoningBank. Use this skill when initializing ReasoningBank database with `npx agentdb@latest init ./.agentdb/reasoningbank.db --dimension 1536` for OpenAI embeddings or custom dimensions, starting MCP server for Claude Code integration with `npx agentdb@latest mcp` then `claude mcp add agentdb npx agentdb@latest mcp`, migrating from legacy ReasoningBank with `npx agentdb@latest migrate --source .swarm/memory.db` automatically converting old data with validation, recording experiences with `reasoningBank.recordExperience({ task, approach, outcome, context })` capturing decisions and results, judging verdicts by evaluating whether outcomes met success criteria using `reasoningBank.judge({ experience_id, verdict: 'success' | 'failure', metrics })`, distilling memories with `reasoningBank.distill()` condensing similar experiences into generalized patterns for faster retrieval, recognizing patterns with `reasoningBank.findSimilar({ task, context, k: 5 })` retrieving experiences similar to current situation, implementing trajectory tracking recording complete decision paths from initial state through actions to final outcome, optimizing decision-making by learning which approaches work best in specific contexts through experience replay, leveraging 150x faster pattern retrieval (<100µs), 500x faster batch operations (2ms for 100 vectors), and <1ms memory access for real-time agent responses, creating self-learning agents that improve performance over time by analyzing successes and failures, implementing experience replay systems for reinforcement learning training on historical trajectories, using AgentDB adapter with `createAgentDBAdapter({ dbPath: './.agentdb/reasoningbank.db', dimensions: 1536 })` for persistence, migrating seamlessly with 100% backward compatibility preserving all legacy ReasoningBank data and APIs, monitoring learning progress with `npx agentdb@latest stats` showing pattern counts, success rates, and memory usage, or troubleshooting migration errors, slow pattern retrieval, verdict judgment failures, or memory distillation issues. Essential for building agents that learn from experience rather than static programming, optimizing decision-making through analysis of historical outcomes, implementing reinforcement learning with experience replay for training efficiency, maintaining persistent knowledge across agent restarts and sessions, improving agent performance systematically by identifying successful patterns, and scaling adaptive learning systems to millions of experiences with AgentDB's performance optimizations."
---
