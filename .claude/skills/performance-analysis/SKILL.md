...
version: 1.0.0
description: Comprehensive performance analysis, bottleneck detection, and optimization recommendations for Claude Flow swarms identifying performance issues across communication, processing, memory, and network with real-time monitoring, historical analysis, detailed report generation in multiple formats (HTML, JSON, CSV), and AI-powered optimization suggestions. Use this skill when detecting bottlenecks with `npx claude-flow bottleneck detect` identifying slow communication between agents, processing delays, memory pressure, or network latency, generating performance reports with `npx claude-flow analysis performance-report --format html --include-metrics` creating comprehensive analyses with visualizations and trends, auto-fixing detected issues with `npx claude-flow bottleneck detect --fix --threshold 15` automatically applying optimizations when performance drops below threshold percentile, profiling swarm operations with real-time monitoring showing agent utilization, task queue depths, message latency, and throughput metrics, analyzing historical performance data with `npx claude-flow analysis performance-report --time-range 7d` tracking trends over days/weeks, identifying communication bottlenecks when message passing between agents exceeds latency thresholds or queue backlogs develop, detecting processing bottlenecks when CPU-intensive operations block agents or tasks timeout frequently, recognizing memory bottlenecks when agent memory usage exceeds limits or garbage collection causes pauses, spotting network bottlenecks when inter-node communication latency spikes or bandwidth saturation occurs, generating optimization recommendations with AI-powered analysis suggesting topology changes (mesh → hierarchical), agent count adjustments, caching strategies, or workload rebalancing, creating performance reports in HTML format with interactive charts, JSON for programmatic analysis, or CSV for spreadsheet import, tracking performance metrics over time including p50/p95/p99 latency percentiles, throughput (ops/sec), error rates, and resource utilization, implementing automated fixes when bottlenecks detected including adding agents, redistributing workload, enabling caching, or adjusting communication patterns, monitoring swarm health with real-time dashboards showing current bottlenecks, agent status, task queue sizes, and system resource usage, exporting metrics for integration with external monitoring systems (Prometheus, Grafana, Datadog), or troubleshooting performance degradation, unexplained latency spikes, memory leaks in long-running swarms, or coordination overhead in large-scale deployments. Essential for maintaining optimal swarm performance at scale, identifying and resolving bottlenecks before they impact productivity, making data-driven decisions about swarm topology and agent allocation, ensuring SLAs are met through continuous performance monitoring, optimizing resource utilization and reducing infrastructure costs, and debugging complex performance issues in distributed multi-agent systems.
category: monitoring
