...
description: "Implement semantic vector search with AgentDB for intelligent document retrieval using embedding-based similarity, similarity matching across text, code, images, or multi-modal data, context-aware querying that understands semantic meaning beyond keyword matching, hybrid search combining vector similarity with metadata filters for precise results, and RAG (Retrieval Augmented Generation) patterns for grounding LLM responses in factual knowledge with 150x-12,500x faster operations than traditional solutions. Use this skill when initializing vector databases with `npx agentdb@latest init ./vectors.db` (default 1536 dimensions for OpenAI ada-002) or `npx agentdb@latest init ./vectors.db --dimension 768` for sentence-transformers or `--dimension 384` for all-MiniLM-L6-v2 models, using preset configurations with `--preset small` (<10K vectors), `--preset medium` (10K-100K vectors), or `--preset large` (>100K vectors) for optimized performance, storing documents with embeddings using `agentdb.vectors.insert({ id, content, embedding: [0.1, 0.2, ...], metadata: { category, date, author } })`, performing semantic search with `npx agentdb@latest query ./vectors.db "[0.1,0.2,0.3]" -k 10 -t 0.75 -m cosine` or `agentdb.vectors.search({ query, k: 10, threshold: 0.7 })` returning top-k most similar results above similarity threshold, implementing hybrid search that filters by metadata fields (category, date, author, tags) before vector similarity ranking with `agentdb.vectors.search({ query, k: 10, filter: { category: 'technical', date: { $gte: '2024-01-01' } } })` for precise domain-specific retrieval, configuring distance metrics for different similarity measures with cosine for text/semantic similarity (default), euclidean for spatial embeddings, or dot product for normalized vectors using `createAgentDBAdapter({ distanceMetric: 'cosine' | 'euclidean' | 'dot' })`, enabling quantization for memory-efficient storage of large embedding collections with `{ quantization: { type: 'scalar', bits: 8 } }` reducing memory by 4x or `{ type: 'binary' }` for 32x reduction, setting up HNSW indexing for 150x-12,500x faster search across millions of vectors with `{ hnsw: { enabled: true, M: 16, efConstruction: 200 } }` achieving <100µs query latency, building RAG systems that retrieve relevant context before LLM generation with `const context = await agentdb.vectors.search({ query: userQuestion, k: 5 }); const response = await llm.generate({ prompt: userQuestion, context })`, integrating with MCP servers for Claude Code using `npx agentdb@latest mcp start --port 8051` or `claude mcp add agentdb npx agentdb@latest mcp` enabling seamless vector search within development workflows, importing existing vector databases from JSON, CSV, or other formats with `npx agentdb@latest import embeddings.json --db ./vectors.db`, exporting search indices for backup or deployment with `npx agentdb@latest export --output vectors.json --format json`, using in-memory databases for testing with `npx agentdb@latest init ./vectors.db --in-memory` for fast prototyping, or troubleshooting search quality issues like irrelevant results, slow queries, or poor ranking through relevance tuning, index optimization, or embedding model selection. Essential for building semantic search engines that understand meaning beyond keywords, implementing RAG systems that ground AI responses in accurate source material, creating intelligent documentation search that finds conceptually similar content across large codebases, enabling multi-modal retrieval across text, code, images, and audio with unified vector representations, and achieving production-grade search performance with sub-millisecond latency across millions of documents while maintaining high recall and precision."
---
