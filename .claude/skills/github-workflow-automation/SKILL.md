...
version: 1.0.0
category: github
description: Advanced GitHub Actions workflow automation with AI swarm coordination creating intelligent, self-organizing CI/CD pipelines that analyze codebases, detect languages and frameworks, suggest optimal build strategies, implement parallelization, configure security scans, and orchestrate complex deployment workflows. Use this skill when generating workflows with `npx ruv-swarm actions generate-workflow --analyze-codebase --detect-languages --create-optimal-pipeline` auto-detecting Node.js/Python/Go projects and creating test/build/deploy steps, optimizing existing workflows with `npx ruv-swarm actions optimize --workflow ".github/workflows/ci.yml" --suggest-parallelization` identifying opportunities for parallel test matrices or dependency caching, creating comprehensive CI pipelines with `.github/workflows/ci.yml` including linting (ESLint, Pylint, golangci-lint), testing (Jest, pytest, go test), security scanning (npm audit, Snyk, Trivy, CodeQL), and build validation, implementing CD pipelines with staging/production deployments triggered by branch protection or manual approvals using environment secrets and gates, configuring matrix strategies with `strategy: matrix: node: [16, 18, 20], os: [ubuntu-latest, windows-latest, macos-latest]` testing across versions and platforms, setting up caching with `actions/cache@v3` for node_modules, pip packages, Go modules reducing build times 50-80%, creating reusable workflows with `.github/workflows/reusable-deploy.yml` and `workflow_call` for DRY composition, orchestrating multi-repo workflows with `repository_dispatch` events triggering dependent repo builds, configuring conditional execution with `if: github.event_name == 'pull_request'` running expensive jobs only when needed, implementing deployment strategies (blue-green, canary with traffic shifting, rollback on failed health checks), managing workflow secrets with GitHub encrypted secrets, environment variables, OIDC token auth eliminating long-lived credentials, setting up status checks as required before merge with branch protection rules and CODEOWNERS, creating custom GitHub Actions with TypeScript or Docker containers, monitoring execution with `gh run list --workflow=ci.yml` and `gh run view 12345` analyzing logs and timing, implementing auto-merge with `gh pr merge 123 --auto --squash` when checks pass, configuring workflow permissions with `permissions: contents: read, pull-requests: write` for least privilege, or troubleshooting workflow failures, timeouts, flaky tests, caching issues, or cloud deployment auth errors. Essential for automating software development lifecycle with CI/CD best practices, eliminating manual testing/deployment prone to errors, scaling build infrastructure with GitHub-hosted or self-hosted runners, implementing security controls with automated scanning and approval gates, optimizing developer productivity through caching and parallelization, and creating self-organizing workflows that adapt to codebase changes using AI swarm intelligence. Requires GitHub CLI (gh) with repo admin access, git, claude-flow@alpha for swarm coordination, Node.js v16+, high priority for production systems, progressive disclosure for gradual learning.
tags:
  - github
  - github-actions
  - ci-cd
