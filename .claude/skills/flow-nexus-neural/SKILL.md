...
description: Train and deploy neural networks in distributed E2B sandbox environments with Flow Nexus cloud platform supporting multiple architectures (feedforward, LSTM, GAN, autoencoder, transformer), distributed training across multiple nodes for parallel processing, hyperparameter optimization with grid search and Bayesian optimization, training tiers from nano to large for different resource requirements, model checkpointing and versioning, and integration with Flow Nexus marketplace for pre-built model templates. Use this skill when training custom neural networks with `mcp__flow-nexus__neural_train({ architecture: 'feedforward' | 'lstm' | 'gan' | 'autoencoder' | 'transformer', config: {...}, data: [...] })` for single-node training or `mcp__flow-nexus__neural_train_distributed({ nodes: 4, architecture: 'transformer', config: {...} })` for distributed multi-node parallel training across cloud sandboxes, deploying trained models to production with `mcp__flow-nexus__neural_deploy({ model_id, tier: 'nano' | 'mini' | 'small' | 'medium' | 'large' })` selecting appropriate compute resources, monitoring training progress with real-time metrics like loss curves, accuracy, validation scores, and resource utilization via `mcp__flow-nexus__neural_status({ job_id })`, implementing hyperparameter optimization to find best learning rates, batch sizes, network depths, and activation functions with automated grid search or Bayesian optimization, managing model versions and checkpoints with `mcp__flow-nexus__neural_checkpoint({ model_id, version })` for experiment tracking and rollback capabilities, leveraging distributed training for large datasets or complex architectures that require parallel processing across multiple GPU-enabled sandbox nodes, using pre-built templates from Flow Nexus marketplace with `mcp__flow-nexus__template_deploy({ template_id: 'bert-fine-tuning' | 'image-classification' | 'time-series-forecasting' })` for common ML tasks, configuring training parameters like epochs, batch size, learning rate, optimizer (Adam, SGD, RMSprop), loss functions (cross-entropy, MSE, custom), and regularization (dropout, L1/L2), integrating with existing datasets via `mcp__flow-nexus__neural_data_upload()` or streaming data from cloud storage, or troubleshooting training issues like vanishing gradients, overfitting, slow convergence, or out-of-memory errors through sandbox resource scaling. Essential for building production ML systems without managing infrastructure, scaling neural network training from prototypes to production-grade models, implementing distributed training for large-scale deep learning, experimenting with different architectures and hyperparameters efficiently, deploying trained models with automatic scaling and monitoring, and leveraging cloud-based GPU resources for cost-effective ML development. Requires Flow Nexus authentication via `npx flow-nexus@latest register` and `npx flow-nexus@latest login`, and Flow Nexus MCP server installation with `claude mcp add flow-nexus npx flow-nexus@latest mcp start`.
version: 1.0.0
category: ai-ml
tags:
  - neural-networks
