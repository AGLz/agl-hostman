...
description: AI-assisted pair programming with multiple collaboration modes (driver, navigator, switch, TDD, review, mentor, debug), real-time quality verification with automatic rollback on failures, intelligent role management with seamless driver/navigator switching, comprehensive testing integration (auto-generate tests, track coverage, continuous testing), continuous code review (security scanning, performance analysis, best practice enforcement), and session persistence (auto-save, recovery, export, sharing). Use this skill when starting pair programming sessions with `claude-flow pair --start` for simple collaboration or `claude-flow pair --start --mode tdd --test-first --coverage 90` for test-driven development, switching between driver mode (implementing code with AI navigating/reviewing) and navigator mode (AI implements while you review/guide) with `claude-flow pair --switch-role`, implementing TDD workflows with test-first approach where tests are written before implementation code, running continuous quality verification with truth-score thresholds (default 0.95) automatically rolling back changes that fail verification, performing real-time code review with security scanning (SQL injection, XSS, hardcoded secrets), performance analysis (N+1 queries, inefficient algorithms), and best practice enforcement (SOLID principles, DRY, proper error handling), auto-generating tests for new code with coverage tracking ensuring minimum thresholds are met, conducting debugging sessions where AI assists with error analysis, stack trace interpretation, and root cause identification, running refactoring sessions with safety nets ensuring behavior preservation through automated testing, implementing mentor mode where AI explains concepts, suggests improvements, and teaches best practices, managing session persistence with auto-save for crash recovery, export for sharing with team members, and resumption for continuing work across sessions, tracking quality metrics including test coverage percentages, truth-scores over time, code complexity trends, and technical debt accumulation, configuring linters (ESLint, pylint) and formatters (Prettier, Black) for automatic code style enforcement, integrating with testing frameworks (Jest, pytest, RSpec) for comprehensive test execution, implementing continuous testing where tests run automatically as code changes, monitoring performance with real-time feedback on algorithm efficiency, memory usage, and execution time, or troubleshooting verification failures, role switching issues, test generation errors, or session recovery problems. Essential for maintaining high code quality through continuous AI-assisted review, accelerating development with real-time feedback and automated test generation, learning best practices and design patterns through mentor mode guidance, ensuring code correctness with truth-score verification and automatic rollback, implementing robust test-driven development workflows, debugging complex issues with AI assistance for faster resolution, and building maintainable codebases through disciplined pair programming practices.
---
