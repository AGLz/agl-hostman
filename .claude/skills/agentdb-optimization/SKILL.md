...
description: "Optimize AgentDB performance with quantization techniques providing 4-32x memory reduction (binary quantization for 32x, scalar quantization for 4x, product quantization for 8-16x), HNSW (Hierarchical Navigable Small World) indexing delivering 150x-12,500x faster search with O(log n) complexity and <100µs query latency, intelligent caching with LRU strategy for 1000 most-used patterns by default, batch operations providing 500x speedup through vectorized processing (2ms for 100 vectors vs 1s sequential), memory consolidation for duplicate pattern removal, and pruning strategies for removing low-value data. Use this skill when running comprehensive performance benchmarks with `npx agentdb@latest benchmark` showing pattern search 150x faster (100µs vs 15ms), batch insert 500x faster, and large-scale query 12,500x faster (8ms vs 100s at 1M vectors), implementing quantization to reduce memory footprint with `createAgentDBAdapter({ quantizationType: 'binary' | 'scalar' | 'product', quantizationBits: 8 })`, enabling HNSW indexing for ultra-fast nearest neighbor search with configurable parameters `{ hnsw: { M: 16, efConstruction: 200, efSearch: 100 } }` where M controls graph connectivity (typical 12-48), efConstruction affects build time vs quality tradeoff (200-400 recommended), and efSearch balances speed vs accuracy at query time (50-200), setting up caching to accelerate repeated queries with `{ cache: { enabled: true, maxSize: 1000, ttl: 3600000 } }` for LRU caching of frequently accessed patterns, using batch operations for bulk inserts and updates with `agentdb.batch.insert(vectors)` processing hundreds of vectors in milliseconds through optimized SIMD operations, implementing memory consolidation to merge duplicate or similar patterns and reduce storage overhead by 20-40%, configuring automatic pruning to remove patterns with low access frequency or relevance scores below thresholds for maintaining database health, optimizing for different scales with recipes for small datasets (<10K vectors: no optimization needed, native performance sufficient), medium datasets (10K-100K: scalar quantization + basic HNSW with M=16), large datasets (100K-1M: product quantization + optimized HNSW with M=32, efConstruction=400), or massive datasets (>1M: binary quantization + advanced HNSW with M=48, efConstruction=800 + aggressive caching), tuning HNSW parameters via environment variables `AGENTDB_HNSW_M=16`, `AGENTDB_HNSW_EF_CONSTRUCTION=200`, `AGENTDB_HNSW_EF_SEARCH=100` for optimal performance-accuracy tradeoffs, monitoring performance metrics like query latency percentiles (p50, p95, p99), memory usage, index build time, cache hit rates, and throughput (queries per second) with `agentdb.metrics.report()`, comparing quantization tradeoffs (binary: 32x compression, 95% accuracy; scalar: 4x compression, 99% accuracy; product: 8-16x compression, 97-98% accuracy), or troubleshooting performance issues like slow queries, high memory usage, cache thrashing, index degradation, or suboptimal recall rates. Essential for scaling AI systems to millions of vectors while maintaining sub-millisecond query performance, reducing infrastructure costs through aggressive memory optimization without sacrificing quality, achieving production SLAs with predictable and fast response times (<100µs p99), optimizing for specific hardware constraints (memory-limited embedded systems vs high-performance cloud servers), and building responsive real-time applications that require instant semantic search across vast knowledge bases with minimal latency variance."
---
