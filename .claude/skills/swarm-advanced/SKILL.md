---
name: swarm-advanced
description: Advanced swarm orchestration patterns for distributed research, development, testing, and complex workflows supporting multiple topologies (mesh for peer-to-peer research/brainstorming, hierarchical for structured development, ring for sequential processing, star for centralized control), dynamic agent spawning and coordination, parallel task execution, and comprehensive workflow management using both MCP tools and CLI commands. Use this skill when initializing swarm topologies with `mcp__claude-flow__swarm_init({ topology: 'mesh' | 'hierarchical' | 'ring' | 'star', maxAgents: 6 })` or `npm install -g claude-flow@alpha && claude mcp add claude-flow npx claude-flow@alpha mcp start` for MCP setup, spawning specialized agents with `mcp__claude-flow__agent_spawn({ type: 'researcher' | 'coder' | 'tester', name: 'Agent 1' })` based on workload requirements, orchestrating distributed tasks with `mcp__claude-flow__task_orchestrate({ task: '...', strategy: 'parallel' | 'sequential' | 'adaptive' })` handling complex multi-step workflows, implementing mesh topology for research and analysis where all agents communicate directly with high flexibility and resilience, using hierarchical topology for development and structured workflows with clear command structure and coordinator-subordinate relationships, employing ring topology for sequential processing where tasks flow agent-to-agent in circular pattern, applying star topology for centralized control where one coordinator manages all worker agents, coordinating parallel research where multiple agents investigate different approaches simultaneously then synthesize findings, managing distributed development with agents handling frontend, backend, testing, documentation concurrently, implementing complex testing workflows with test generation, execution, analysis, and reporting across specialized agents, executing multi-phase workflows (research → design → implement → test → deploy) with agent handoffs between phases, dynamically spawning agents based on workload spikes or task complexity, balancing load across agents monitoring utilization and redistributing work, implementing fault tolerance with agent health monitoring and automatic replacement of failed agents, coordinating cross-agent communication through message passing, shared memory, or event broadcasting, managing workflow state with checkpoints allowing resume after interruptions, tracking swarm metrics including agent utilization, task completion rates, communication overhead, and bottlenecks, scaling swarms dynamically from 2 agents to hundreds based on task requirements, or troubleshooting topology mismatches (wrong topology for task type), agent coordination failures, communication bottlenecks, load imbalances, or fault recovery issues. Essential for building scalable distributed AI systems with appropriate topology selection, coordinating complex multi-phase development workflows across specialized agents, implementing robust fault-tolerant systems with automatic recovery, optimizing resource utilization through dynamic agent spawning and load balancing, scaling from simple tasks to complex enterprise workflows, maintaining high availability through redundant agent coordination, and achieving maximum productivity through intelligent parallelization and task distribution.
version: 2.0.0
