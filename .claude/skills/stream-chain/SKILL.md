---
name: stream-chain
description: Stream-JSON chaining for multi-agent pipelines enabling sophisticated data transformation through sequential workflows where each agent's output flows directly into the next agent's input, supporting custom prompt sequences with full control or predefined battle-tested pipelines for common tasks with streaming data flow coordination. Use this skill when running custom chains with `claude-flow stream-chain run "Analyze codebase structure" "Identify improvement areas" "Generate action plan"` executing sequential prompts where each step receives complete output from previous step, executing predefined pipelines with `claude-flow stream-chain pipeline analysis` using battle-tested workflows for common tasks (analysis, refactoring, documentation, testing), creating sophisticated multi-step workflows where agent outputs flow seamlessly into subsequent agent inputs enabling complex transformations, implementing data transformation pipelines where raw data → processed data → analyzed data → actionable insights through sequential agent processing, orchestrating multi-agent coordination through streaming where agents collaborate by passing results forward rather than shared memory, building custom chains for specific workflows like "research API patterns" → "design implementation" → "generate tests" → "create documentation", leveraging predefined pipelines including analysis (code structure → improvements → action plan), refactoring (identify issues → design changes → implement → verify), documentation (analyze code → extract concepts → write docs → generate examples), testing (analyze coverage → identify gaps → generate tests → verify execution), optimizing sequential processing where each agent specializes in one transformation step rather than handling entire workflow, implementing progressive refinement where each chain step improves upon previous output, managing complex data flows where structured JSON passes between agents maintaining context and intermediate results, coordinating asynchronous workflows where agents process in sequence but can run in parallel when dependencies allow, creating reusable pipeline templates for organization-wide standardized workflows, monitoring pipeline execution with intermediate outputs visible for debugging and validation, handling errors gracefully with rollback to last successful step or alternative pathways, or troubleshooting pipeline failures, chain interruptions, data transformation errors, or agent coordination issues. Essential for building sophisticated multi-step AI workflows with clear data flow, implementing complex transformations through sequential agent specialization, coordinating multi-agent systems without shared state management overhead, creating reproducible workflows through predefined pipelines, scaling analysis and transformation tasks across specialized agents, maintaining clarity in complex operations through explicit sequential steps, and achieving modularity where each agent focuses on single responsibility in transformation chain.
version: 1.0.0
category: workflow
