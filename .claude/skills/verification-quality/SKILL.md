...
description: "Comprehensive truth scoring, code quality verification, and automatic rollback system with 0.95 accuracy threshold ensuring high-quality agent outputs through real-time reliability metrics (0.0-1.0 scale), automated code correctness/security/best practices validation, instant reversion of changes failing verification, statistical analysis with trends and confidence intervals, CI/CD integration capabilities, and real-time monitoring dashboards. Use this skill when viewing truth scores with `npx claude-flow@alpha truth` showing current reliability metrics for code, agents, and tasks, running verification checks with `npx claude-flow@alpha verify check` validating code correctness, security vulnerabilities, and best practice compliance, verifying specific files with `npx claude-flow@alpha verify check --file src/app.js --threshold 0.98` using custom quality thresholds, rolling back failed changes with `npx claude-flow@alpha verify rollback --last-good` reverting to previous verified state, implementing automatic quality gates where changes below 0.95 threshold (default) are automatically rolled back preventing bad code from persisting, tracking truth scores over time with statistical analysis showing trends, p-values, confidence intervals, and improvement trajectories, generating quality metrics including code correctness scores, security vulnerability counts, best practice violations, and technical debt indicators, integrating with CI/CD pipelines exporting metrics for continuous integration systems (Jenkins, GitHub Actions, CircleCI), monitoring real-time quality with live dashboards showing current scores, recent verifications, rollback history, and agent performance, implementing git integration where rollbacks use git reset to restore last good commit before failed verification, configuring verification rules checking linting errors, test failures, security scans (npm audit, Snyk), type checking (TypeScript), and code complexity, setting custom thresholds with --threshold parameter for different quality requirements (0.98 for critical code, 0.90 for experimental features), tracking agent quality scores identifying which agents produce highest/lowest quality outputs for optimization, generating quality reports with historical trends, improvement recommendations, and actionable insights, implementing pre-commit hooks running verification before allowing commits ensuring quality gates at commit time, monitoring verification history with timestamps, scores, outcomes (pass/fail), and rollback actions, or troubleshooting false positives in verification, rollback failures, threshold misconfiguration, or CI/CD integration issues. Essential for maintaining high code quality standards automatically without manual review overhead, preventing low-quality code from entering codebase through automatic rollback, tracking code quality trends over time identifying improvements or degradations, ensuring agent outputs meet reliability standards, integrating quality assurance into development workflows with automated verification, providing objective quality metrics for team performance tracking, implementing safety nets for AI-generated code, and achieving production-grade code quality through systematic verification and validation.
version: "2.0.0"
