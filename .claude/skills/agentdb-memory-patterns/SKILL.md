...
description: "Implement persistent memory patterns for AI agents using AgentDB with session memory for conversation context, long-term memory for knowledge retention across interactions, pattern learning from historical data, context management for multi-turn dialogues, and integration with 9 reinforcement learning algorithms and 4 reasoning modules through ReasoningBank. Use this skill when initializing agent memory systems with `npx agentdb@latest init ./agents.db` or `agentdb.memory.init({ dimensions: 768 })`, storing and retrieving conversation context using `agentdb.memory.store({ sessionId, content, metadata })` and `agentdb.memory.recall({ sessionId, query, k: 10 })` for context-aware responses, implementing long-term knowledge retention that persists across agent restarts and multiple sessions with automatic pattern consolidation, querying agent memories with semantic search using `npx agentdb@latest query "./agents.db" "user preferences" -k 5` to find relevant historical interactions, importing existing knowledge bases from JSON, CSV, or other formats with `npx agentdb@latest import data.json --db ./agents.db`, exporting agent memories for backup, analysis, or migration using `npx agentdb@latest export --output backup.json --format json`, setting up MCP (Model Context Protocol) servers for Claude Code integration with `npx agentdb@latest mcp start --port 8051` enabling seamless memory access within development workflows, creating learning plugins with `npx agentdb@latest create-plugin -t decision-transformer -n agent-memory` for reinforcement learning, migrating from ReasoningBank to AgentDB while preserving learning algorithms (Decision Transformer, Q-Learning, SARSA, Actor-Critic, Active Learning, Adversarial Training, Curriculum Learning, Federated Learning, Multi-Task Learning) and reasoning modules (causal, deductive, inductive, analogical), leveraging ultra-fast retrieval with 150x-12,500x performance improvements and <100µs search latency for real-time agent responses, configuring memory presets with `--preset small|medium|large` for different scales (<10K, 10K-100K, >100K memories), or building chat systems, virtual assistants, customer support bots, or intelligent agents that remember user preferences, conversation history, learned behaviors, and contextual patterns. Essential for creating stateful AI agents that maintain coherent long-term interactions, implementing context-aware responses based on historical conversations, building intelligent systems that learn and improve from experience with reinforcement learning, enabling seamless knowledge transfer between agent instances, and achieving production-grade performance with sub-millisecond memory retrieval in real-time applications."
---
