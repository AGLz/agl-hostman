...
version: 2.0.0
description: Comprehensive GitHub release orchestration with AI swarm coordination for automated versioning (semantic versioning, conventional commits), changelog generation from commit history, build artifact creation, multi-platform deployment (npm, Docker, GitHub releases), testing pipelines with quality gates, and rollback management with version history tracking. Use this skill when creating draft releases with `gh release create v2.0.0 --draft --generate-notes --title "Release v2.0.0"` for manual review before publishing, orchestrating complete releases with `npx claude-flow github release-create --version "2.0.0" --build-artifacts --deploy-targets "npm,docker,github"` coordinating build, test, and deployment across platforms, initializing release swarms with `npx claude-flow swarm init --topology hierarchical` then executing full pipelines, generating changelogs from conventional commits (feat:, fix:, breaking:), implementing semantic versioning with automated version bumps, building release artifacts (npm packages, Docker images, binaries), running pre-release testing with quality gates (coverage >80%, 0 lint errors, security scans), deploying to multiple platforms simultaneously, managing release notes with breaking changes and linked issues, coordinating multi-repo releases with version alignment, implementing rollback procedures, tracking release metrics (deployment frequency, MTTR, change failure rate), automating release schedules with cron triggers, implementing gradual rollouts (canary, blue-green), creating release checklists, integrating with notification systems (Slack, Discord, webhooks), managing GitHub release assets with checksums and GPG signatures, or troubleshooting failed builds, deployment errors, version conflicts, or rollback procedures. Essential for automating software release processes, coordinating complex multi-platform releases, ensuring release quality through automated testing, implementing continuous delivery, managing rollbacks efficiently, and providing release visibility with automated changelogs. Requires GitHub CLI (gh), claude-flow, optional ruv-swarm, optional mcp-github, git, npm/yarn, Node.js >= 20.0.0.
category: github
