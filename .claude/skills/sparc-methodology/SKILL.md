...
description: SPARC (Specification, Pseudocode, Architecture, Refinement, Completion) comprehensive development methodology with multi-agent orchestration providing 17 specialized modes for systematic software development from initial research through deployment and monitoring with test-driven development, parallel agent execution (2.8-4.4x speed improvements), persistent memory integration, and quality-first approach. Use this skill when running complete SPARC workflows with `npx claude-flow sparc tdd "implement user authentication"` executing full TDD pipeline (spec → pseudocode → architecture → test → implement → refine), executing specific phases with `npx claude-flow sparc run spec-pseudocode "feature description"` for specification and algorithm design or `npx claude-flow sparc run architect "system requirements"` for architecture planning, implementing batch execution with `npx claude-flow sparc batch spec-pseudocode,architect,refine "complex feature"` running multiple modes in parallel, running pipeline workflows with `npx claude-flow sparc pipeline "complete project"` executing all phases sequentially with checkpoints, listing available modes with `npx claude-flow sparc modes` showing all 17 specialized modes (spec-pseudocode, architect, tdd, refine, integration, deploy, monitor, test, review, document, optimize, security, performance, migration, legacy, research, prototype), getting mode details with `npx claude-flow sparc info tdd` showing specific mode capabilities and options, implementing test-driven development where tests are written before implementation code ensuring correctness, leveraging parallel execution with concurrent agent coordination for 2.8-4.4x speed improvements over sequential workflows, integrating persistent memory with knowledge sharing across agents and sessions via `mcp__claude-flow__memory_usage`, following systematic approach with structured phases (Specification: requirements analysis, Pseudocode: algorithm design, Architecture: system design, Refinement: TDD implementation, Completion: integration and deployment), emphasizing quality-first with comprehensive reviews, testing at every phase, and validation before proceeding, implementing modular design with clean separation of concerns and clear interfaces between components, coordinating multi-agent workflows where specialized agents handle different SPARC phases concurrently, managing complex projects with 17 modes covering entire development lifecycle from research to production monitoring, using orchestration patterns (mesh for research/brainstorming, hierarchical for structured development, adaptive for dynamic workflows), integrating with build systems (npm run build, npm test, npm run lint, npm run typecheck), or troubleshooting phase transitions, agent coordination failures, test failures blocking refinement, or integration issues. Essential for implementing systematic development methodologies with proven structure, scaling development through parallel multi-agent execution, ensuring code quality with test-driven development practices, managing complex software projects with clear phases and checkpoints, coordinating distributed development teams with shared methodology, maintaining consistency across large codebases through systematic approaches, and achieving 2.8-4.4x productivity improvements through intelligent parallelization.
version: 2.7.0
