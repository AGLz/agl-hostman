...
version: 1.0.0
description: Comprehensive GitHub pull request code review with AI-powered swarm coordination deploying specialized review agents for security analysis, performance optimization, code style checking, architecture evaluation, and accessibility compliance that goes beyond traditional static analysis. Use this skill when reviewing pull requests with multi-agent orchestration using `gh pr view 123 --json files,diff | npx ruv-swarm github review-init --pr 123` to initialize intelligent review swarms, performing comprehensive reviews with `npx ruv-swarm github review-init --pr 123 --agents "security,performance,style,architecture,accessibility" --depth comprehensive` analyzing code changes across multiple quality dimensions, generating intelligent PR comments with `gh pr comment 123 --body "🔍 Multi-agent code review initiated"` providing context-aware feedback, enforcing quality gates with automated checks for test coverage, security vulnerabilities (OWASP Top 10), performance regressions, and code style violations, analyzing security concerns with specialized agents that detect SQL injection, XSS, authentication flaws, insecure dependencies, and hardcoded secrets, evaluating performance implications with agents checking for N+1 queries, inefficient algorithms (O(n²) vs O(n log n)), memory leaks, and blocking operations, validating code style compliance with ESLint, Prettier, RuboCop, or Black configurations and architectural patterns (SOLID principles, DRY, separation of concerns), reviewing accessibility standards with WCAG 2.1 compliance checks for ARIA attributes, keyboard navigation, color contrast (4.5:1), and semantic HTML, orchestrating swarm-based reviews where each agent specializes in specific code quality aspects and collaborates through shared context, posting consolidated review summaries with `gh pr review 123 --comment --body "$(npx ruv-swarm github review-summary)"` aggregating findings from all specialized agents, integrating with GitHub Actions workflows for automated PR review triggers using `.github/workflows/code-review.yml` with `on: pull_request` events, managing review lifecycle with draft reviews, requested changes, approval workflows, and re-review triggers after code updates, tracking review metrics like review time, comment density, approval rates, and defect detection effectiveness, or troubleshooting review bottlenecks, false positives from static analysis, or coordination issues between multiple review agents. Essential for maintaining high code quality standards across teams, catching security vulnerabilities and performance issues early in development, automating repetitive review tasks while preserving human judgment for complex decisions, scaling code review capacity through intelligent agent delegation, enforcing consistent coding standards and architectural patterns organization-wide, and improving review turnaround time from hours to minutes through parallel agent analysis. Requires GitHub CLI (`gh`) authenticated with repository access, ruv-swarm or claude-flow MCP server for swarm coordination, and appropriate repository permissions for commenting and review submission.
category: github
