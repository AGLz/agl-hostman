...
version: 2.0.0
category: github
description: Comprehensive GitHub project management with swarm-coordinated issue tracking, automated project board synchronization, sprint planning with velocity tracking, milestone management, and intelligent task decomposition using AI agents to organize issues, update project boards, track progress, and coordinate agile workflows across repositories. Use this skill when creating coordinated issues with `gh issue create --title "Feature: Advanced Authentication" --body "Implement OAuth2..." --label "enhancement,swarm-ready"` that automatically trigger swarm agents for task breakdown, managing GitHub Projects with `gh project list` and `gh project item-list 1` viewing boards and backlog items, automating board updates with `gh project item-edit --project-id 1 --id 123 --field-id Status --text "In Progress"` when PRs are opened or issues transition, planning sprints with `gh issue list --milestone "Sprint 23" --state all --json number,title,labels,assignees` analyzing sprint capacity and velocity, decomposing complex issues into sub-tasks with swarm agents that analyze issue descriptions and generate actionable checklist items using `mcp__claude-flow__task_orchestrate()`, tracking progress with automated status updates when linked PRs are merged or commits reference issue numbers using GitHub issue keywords (fixes #123, closes #456), coordinating cross-repository issues with `gh issue create --repo org/backend --title "API: Add /auth endpoint" --body "Related to org/frontend#789"` linking frontend and backend work, managing milestones with `gh milestone create "v2.0.0" --due-date 2024-06-01` and tracking completion percentage, generating burndown charts with `gh issue list --milestone "Sprint 23" --state closed --json closedAt` calculating daily progress, using project board views (board, table, roadmap) with custom fields like Priority (P0-P3), Story Points (1,2,3,5,8), Team (Frontend, Backend, DevOps), automating issue triage with label classification using `gh issue edit 123 --add-label "needs-investigation"` based on title/body content analysis, implementing GitHub Projects V2 workflows with custom fields, single-select options, iteration fields, and automated item archival, coordinating agile ceremonies with automated sprint retrospective issue creation, velocity calculations from closed story points, and capacity planning based on team availability, integrating with external tools via GraphQL API mutations for project item updates, field value changes, and issue assignments, managing dependencies between issues with task lists (`- [ ] Implement feature`, `- [x] Write tests`) and blocking relationships, synchronizing TodoWrite todos with GitHub issues bidirectionally keeping local task lists and remote issues aligned, or troubleshooting project board sync delays, incorrect issue status transitions, milestone calculation errors, or swarm coordination failures. Essential for managing software projects with GitHub-native tools avoiding external project management systems, coordinating agile development with sprint planning, velocity tracking, and burndown charts, automating repetitive project management tasks (status updates, board organization, issue triage), scaling project tracking across multiple repositories in monorepo or microservices architectures, providing visibility into team progress with automated reporting and dashboards, and leveraging AI swarm intelligence for intelligent task breakdown and work estimation. Requires GitHub CLI (gh) authenticated with repository admin access, ruv-swarm or claude-flow MCP server for swarm coordination, appropriate repository permissions for issue and project management, and tools: mcp__github__*, mcp__claude-flow__*, Bash, Read, Write, TodoWrite. Estimated setup time: 30-45 minutes.
author: Claude Code
tags:
  - github
  - project-management
