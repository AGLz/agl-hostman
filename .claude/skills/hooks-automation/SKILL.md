...
description: Automated coordination, formatting, and learning from Claude Code operations using intelligent hooks with MCP integration providing pre-operation validation and preparation, post-operation formatting and analysis, session state persistence and restoration, memory coordination across swarm agents, Git integration with automated commits, and neural pattern training for continuous improvement. Use this skill when initializing hooks system with `npx claude-flow init --hooks` creating `.claude/settings.json` with pre-configured hooks and default handlers, implementing pre-operation hooks for validation, preparation, and auto-agent assignment before operations using `npx claude-flow hooks pre-task --description "implement feature" --auto-assign`, running post-operation hooks for code formatting (Prettier, ESLint), pattern analysis, and neural training after operations with `npx claude-flow hooks post-edit --file "src/app.js" --format --train-pattern`, managing session state with `npx claude-flow hooks session-start --swarm-id "dev-session"` persisting context and `npx claude-flow hooks session-end --export-metrics` generating summaries, coordinating memory across swarm agents with `npx claude-flow hooks memory-sync --namespace "coordination" --key "swarm/status"` for shared knowledge, integrating with Git workflows using `npx claude-flow hooks pre-commit --verify-tests --check-linting` for quality verification before commits, training neural patterns from successful operations with `npx claude-flow hooks train-pattern --operation "refactor" --success true` for continuous learning, auto-assigning agents based on file type or task complexity, restoring session context after interruptions with `npx claude-flow hooks session-restore --session-id "previous"`, implementing automated code formatting post-edit ensuring consistent style across codebase, tracking performance metrics with token usage, operation timing, and success rates, implementing MCP integration for seamless coordination with claude-flow, ruv-swarm, and flow-nexus MCP servers, caching search results and frequently accessed data for performance optimization, or troubleshooting hook execution failures, memory synchronization delays, Git commit hook errors, or pattern training issues. Essential for automating repetitive development tasks (formatting, linting, testing), maintaining consistent code quality through automated pre/post-operation checks, coordinating knowledge across distributed agent swarms with persistent memory, learning from successful patterns to improve future operations, managing complex development sessions with state persistence and context restoration, integrating seamlessly with Git workflows for quality gates before commits, and optimizing developer productivity through intelligent automation and pattern recognition. Requires Claude Flow CLI (`npm install -g claude-flow@alpha`), Claude Code with hooks enabled, `.claude/settings.json` configuration, optional MCP servers (claude-flow, ruv-swarm, flow-nexus), optional Git repository for version control, and optional testing framework for quality verification.
---
