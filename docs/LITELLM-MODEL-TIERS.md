...
Modelos descobertos automaticamente de `config/litellm/config.yaml` (103 aliases). Filtra por keys em `LITELLM_ENV_FILE` quando `BENCH_REQUIRE_KEYS=1` (default).

**Providers:** ollama, groq, openrouter, zai, anthropic, openai, google, deepseek, moonshot.  
**Nota:** DashScope/Alibaba está desactivado; aliases `qwen*` → Z.AI (ver comentários no config).

```bash
# Regenerar manifest da bateria (todos os modelos do config)
bash scripts/litellm/run-provider-benchmark-suite.sh manifest

# Bateria paralela — testa OK em todos os modelos (tier full ~103 modelos)
bash scripts/litellm/run-provider-benchmark-suite.sh battery --tier full --json

# Benchmark comparativo — latência + capacidade (PONG, raciocínio, JSON, PT)
LITELLM_ENV_FILE=/opt/agl-litellm/.env LITELLM_URL=http://100.125.249.8:4000 \
  BENCH_PROVIDERS=groq,openrouter,zai,anthropic,openai,ollama,google,deepseek \
  BENCH_PROMPTS=latency \
  bash scripts/litellm/run-provider-benchmark-suite.sh compare

# Só free + local (rápido)
BENCH_TIER=free BENCH_PROMPTS=latency \
  bash scripts/litellm/run-provider-benchmark-suite.sh compare

# Suite completa (local + paid + free, 4 prompts)
BENCH_TIER=all BENCH_REQUIRE_KEYS=1 \
  bash scripts/litellm/run-provider-benchmark-suite.sh compare
```

Saída comparativa: `docs/LITELLM-PROVIDER-BENCHMARK.md` + JSON em `/tmp/litellm-provider-benchmark.json`.
