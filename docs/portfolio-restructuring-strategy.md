...
```mermaid
graph TD
    A[Transition Risks] --> B[Market Risk]
    A --> C[Liquidity Risk]
    A --> D[Operational Risk]
    A --> E[Regulatory Risk]
    A --> F[Opportunity Cost]

    B --> B1[Price volatility during rebalancing]
    B --> B2[Market timing risk]

    C --> C1[Asset liquidity constraints]
    C --> C2[Settlement risk]

    D --> D1[System integration]
    D --> D2[Process changes]
    D --> D3[Staff training]

    E --> E1[Compliance gaps]
    E --> E2[Documentation requirements]

    F --> F1[Underperformance during transition]
    F --> F2[Missed opportunities]
```
...
```mermaid
gantt
    title CMN 5.272 Portfolio Restructuring Timeline
    dateFormat YYYY-MM
    axisFormat %b %Y

    section Phase 1: Foundation
    Governance Assessment & Gap Analysis :done, p1, 2026-02, 2M
    System Implementation & Testing :active, p2, 2026-03, 3M
    Staff Training & Certification :p3, 2026-04, 3M

    section Phase 2: Nível I Compliance
    Baseline Portfolio Alignment :crit, p4, 2026-05, 3M
    Documentation & Reporting Setup :p5, 2026-06, 2M
    Nível I Certification :milestone, 2026-08, 0d

    section Phase 3: Nível II Advancement
    Fixed Income Expansion :p6, 2026-08, 4M
    Variable Income Introduction (40%) :crit, p7, 2026-10, 3M
    Nível II Certification :milestone, 2026-12, 0d

    section Phase 4: Nível III Advancement
    Structured Products Integration :p8, 2027-01, 4M
    Real Estate Funds Allocation (20%) :crit, p9, 2027-03, 3M
    Foreign Investment Intro (10%) :p10, 2027-05, 2M
    Nível III Certification :milestone, 2027-06, 0d

    section Phase 5: Nível IV Optimization
    Private Equity/FIP Allocation (10%) :p11, 2027-07, 4M
    International Expansion :crit, p12, 2027-09, 3M
    Payroll Loans Integration (5%) :p13, 2027-11, 2M
    Final Portfolio Optimization :p14, 2028-01, 2M
    Nível IV Certification :milestone, 2028-02, 0d
```
...
```mermaid
pie title Nível I Portfolio Allocation
    "Tesouro Direto/ETFs (100%)" : 70
    "Operações Compromissadas (5%)" : 5
    "Ativos Financeiros IF (20%)" : 15
    "Caixa/Liquidez" : 10
```
...
```mermaid
pie title Nível II Portfolio Allocation
    "Renda Fixa: 60%" : 60
    "Renda Variável: 40%" : 40
```
...
```mermaid
pie title Nível III Portfolio Allocation
    "Renda Fixa: 45%" : 45
    "Renda Variável: 40%" : 40
    "Fundos Imobiliários: 10%" : 10
    "Investimentos Estruturados: 5%" : 5
```
...
```mermaid
pie title Nível IV Portfolio Allocation
    "Renda Fixa: 30%" : 30
    "Renda Variável: 50%" : 50
    "Fundos Imobiliários: 10%" : 10
    "Investimentos Estruturados: 7%" : 7
    "Empréstimos Consignados: 3%" : 3
```
...
```mermaid
graph LR
    A[Portfolio Efficiency] --> B[Return Enhancement]
    A --> C[Risk Reduction]
    A --> D[Diversification Benefits]

    B --> B1[+600 bps cumulative<br/>over 24 months]
    B --> B2[Steady progression<br/>by governance level]

    C --> C1[Volatility optimized<br/>by risk appetite]
    C --> C2[Drawdowns controlled<br/>within limits]

    D --> D1[Asset classes: 5 → 12]
    D --> D2[Geographic: Domestic → Global]
    D --> D3[Currency: BRL → Multi-currency]
```
...
```mermaid
graph TD
    A[Portfolio Optimization] --> B[Efficient Frontier Analysis]
    A --> C[Scenario Testing]
    A --> D[Stress Testing]

    B --> B1[Identify optimal allocation<br/>for each governance level]
    B --> B2[Maximize Sharpe ratio<br/>within constraints]

    C --> C1[Monte Carlo simulation<br/>10,000 iterations]
    C --> C2[Confidence intervals:<br/>95% VaR, 99% CVaR]

    D --> D1[Market crash scenarios:<br/>-20%, -30%, -40%]
    D --> D2[Liquidity stress:<br/> redemption pressure]
    D --> D3[Regulatory changes:<br/>limit adjustments]
```
...
```mermaid
graph TD
    A[Compliance Monitoring] --> B[Daily Checks]
    A --> C[Weekly Reports]
    A --> D[Monthly Reviews]
    A --> E[Quarterly Audits]

    B --> B1[Limit monitoring<br/>real-time alerts]
    B --> B2[Concentration limits<br/>automated checks]
    B --> B3[Liquidity ratios<br/>daily calculations]

    C --> C1[Performance vs<br/>benchmarks]
    C --> C2[Risk metrics<br/>VaR, CVaR, Beta]
    C --> C3[Compliance exceptions<br/>resolution tracking]

    D --> M1[Portfolio rebalancing<br/>recommendations]
    M --> M2[Documentation<br/>completeness check]
    M --> M3[Training completion<br/>status update]

    E --> Q1[External audit<br/>preparation]
    Q --> Q2[Regulatory filing<br/>accuracy review]
    Q --> Q3[Governance advancement<br/>readiness assessment]
```
...
```mermaid
graph TD
    A[Risk Monitoring] --> B[Market Risk]
    A --> C[Liquidity Risk]
    A --> D[Operational Risk]
    A --> E[Compliance Risk]

    B --> B1[Portfolio VaR<br/>Daily calculation]
    B --> B2[Beta tracking<br/>vs. benchmark]
    B --> B3[Stress testing<br/>Weekly scenarios]

    C --> C1[Liquidity ratio<br/>Cash/1-month redemptions]
    C --> C2[Asset liquidity<br/>Tier analysis]
    C --> C3[Concentration<br/>Top 10 positions]

    D --> D1[System uptime<br/>99.9% target]
    D --> D2[Data quality<br/>Automated validation]
    D --> D3[Process SLA<br/>Trade errors <0.1%]

    E --> E1[Limit breaches<br/>Zero tolerance]
    E --> E2[Documentation<br/>100% complete]
    E --> E3[Certification<br/>On-time submission]
```
...
```mermaid
graph LR
    A[Nível I<br/>Months 0-6] --> B[Nível II<br/>Months 6-10]
    B --> C[Nível III<br/>Months 10-16]
    C --> D[Nível IV<br/>Months 16-24]

    A --> A1[Board oversight<br/>Basic risk mgmt]
    B --> B1[Investment Committee<br/>Enhanced reporting]
    C --> C1[Independent members<br/>Advanced analytics]
    D --> D1[External audit<br/>Sophisticated systems]
```
