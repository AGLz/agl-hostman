...
class TerminalServer {
  constructor(options = {}) {
    ...
    // Raiz canónica do repo Evo no contentor: …/dashboard/terminal-server/{bin,src} → /workspace
    this.workspaceRoot = path.resolve(__dirname, '..', '..', '..');
    const envRoot =
      (process.env.WORKSPACE_ROOT && process.env.WORKSPACE_ROOT.trim()) ||
      (process.env.EVONEXUS_WORKSPACE_ROOT && process.env.EVONEXUS_WORKSPACE_ROOT.trim());
    this.baseFolder = envRoot ? path.resolve(envRoot) : this.workspaceRoot;
  ...
  async loadPersistedSessions() {
    try {
      ...
      let normalizedWd = false;
      for (const s of sessions.values()) {
        if (s.superseded == null) {
          s.superseded = false;
        }
        const nextWd = this.normalizePersistedSessionWorkingDir(s.workingDir);
        if (String(s.workingDir || '') !== String(nextWd)) {
          console.log(`[sessions] workingDir normalizado ao carregar: ${s.workingDir} → ${nextWd}`);
          s.workingDir = nextWd;
          normalizedWd = true;
        }
      }
  ...
  /**
   * Espelho legacy `/workspace/workspace` (scripts e SPA por vezes duplicam o segmento).
   * Colapsa para `/workspace/…` para alinhar PTY, Agent SDK (chat) e ferramentas Read/Glob.
   */
  collapseDuplicateWorkspacePath(resolvedPath) {
    const root = path.resolve(this.workspaceRoot);
    const dupRoot = path.join(root, 'workspace');
    const resolved = path.resolve(resolvedPath);
    const rel = path.relative(dupRoot, resolved);
  ...
  normalizePersistedSessionWorkingDir(storedWorkingDir) {
    const fallback = path.resolve(this.baseFolder);
    const raw =
      storedWorkingDir != null && String(storedWorkingDir).trim() !== ''
        ? String(storedWorkingDir).trim()
        : fallback;
    const validation = this.validatePath(raw);
    const canonicalBase = this.validatePath(fallback);
    const resolved =
      validation.valid ? validation.path : canonicalBase.valid ? canonicalBase.path : path.resolve(this.workspaceRoot);
    if (!validation.valid) {
      console.warn(`[sessions] workingDir em disco inválido (${raw}), a usar ${resolved}`);
    }
    return this.collapseDuplicateWorkspacePath(resolved);
  }
  ...
  setupExpress() {
    ...
    this.app.post('/api/sessions/for-agent', (req, res) => {
      ...
      let validWorkingDir = this.collapseDuplicateWorkspacePath(path.resolve(this.baseFolder));
      if (workingDir) {
        ...
        validWorkingDir = this.collapseDuplicateWorkspacePath(validation.path);
      }
      ...
      const ticketScope =
        ticketId !== undefined && ticketId !== null && ticketId !== ''
          ? ticketId
          : null;
      this.markInactiveSessionsSuperseded(agentName, validWorkingDir, ticketScope);
    ...
    this.app.post('/api/sessions/create', (req, res) => {
      ...
      this.markInactiveSessionsSuperseded(agentName, validWorkingDir);
    ...
    this.app.delete('/api/sessions/:sessionId', (req, res) => {
      ...
      // Evita SIGTERM no PTY a meio de um turno quando o SPA faz "nova sessão" e apaga
      // a anterior em background sem o utilizador carregar explicitamente em apagar.
      if (session.active && !force) {
        console.warn(
          `[sessions] DELETE ${sessionId} refused (active=true, connections=${session.connections.size}); use ?force=1 to destroy`,
        );
        return res.status(409).json({
          error: 'session_active',
          message:
            'Esta sessão tem o Claude Code em execução. Para o agente no terminal antes de remover, ou DELETE com ?force=1.',
        });
      }

      console.warn(
        `[sessions] DELETE ${sessionId} agent=${session.agentName || ''} active=${session.active} connections=${session.connections.size} force=${Boolean(force)}`,
      );
      ...
      case 'chat_send':
        ...
        if (wsInfo.claudeSessionId) {
          ...
          if (chatSession && data.prompt) {
            ...
            if (!chatSession.sdkSessionId && chatSession.chatHistory.length > 1) {
              const recentHistory = chatSession.chatHistory.slice(-12, -1).map((m) => {
                if (m.role === 'user') return `User: ${m.text || ''}`;
                if (m.role === 'assistant') {
                  const text = Array.isArray(m.blocks)
              ...
              if (recentHistory.trim()) {
                promptForModel = [
                  'Contexto recente desta conversa no dashboard. Use isto para continuar a tarefa, pois a sessão Claude foi reiniciada sem resume técnico:',
                  recentHistory,
                  '',
            ...
            try {
              await this.chatBridge.startSession(wsInfo.claudeSessionId, {
                ...
                onMessage: (msg) => {
                  ...
                  if (msg.type === 'assistant_message' && Array.isArray(msg.blocks) && msg.blocks.length > 0 && assistantBlocks.length === 0) {
                    assistantBlocks = msg.blocks.map((block) => {
                      if (block.type === 'tool_use') return { ...block, done: true };
                      return block;
                    });
                  }

                  if (msg.type === "result" && msg.resultText && assistantBlocks.length === 0) {
                    assistantBlocks.push({ type: "text", text: msg.resultText });
                  }
  ...
  async joinClaudeSession(wsId, claudeSessionId) {
    ...
    const normalizedWd = this.normalizePersistedSessionWorkingDir(session.workingDir);
    if (normalizedWd !== session.workingDir) {
      console.log(`[sessions] workingDir normalizado ao join: ${session.workingDir} → ${normalizedWd}`);
      session.workingDir = normalizedWd;
      void this.saveSessionsToDisk();
    }
  ...
  /**
   * Marca sessões inativas como obsoletas quando se cria uma nova para o mesmo agente
   * e diretório — evita start_claude num PTY antigo (EIO / SIGTERM) quando o UI já
   * tem um sessionId novo.
   *
   * @param {string} agentName
   * @param {string} validWorkingDir
   * @param {string|undefined|null} ticketScope — `undefined`: todas as inativas com o mesmo agente+wd;
   *   `null`: só sessões sem ticket; string: mesmo ticketId
   */
  markInactiveSessionsSuperseded(agentName, validWorkingDir, ticketScope = undefined) {
    for (const s of this.claudeSessions.values()) {
      if (s.agentName !== agentName) {
        continue;
      }
      if (String(s.workingDir || '') !== String(validWorkingDir || '')) {
        continue;
      }
      if (ticketScope !== undefined) {
        let ticketMatch;
        if (ticketScope === null) {
          ticketMatch = !s.ticketId;
        } else {
          ticketMatch = s.ticketId === ticketScope;
        }
        if (!ticketMatch) {
          continue;
        }
  ...
  findNewestNonSupersededByAgentAndWd(agentName, workingDir) {
    let best = null;
    for (const s of this.claudeSessions.values()) {
      if (s.superseded) {
        continue;
      ...
      if (String(s.workingDir || '') !== String(workingDir || '')) {
        continue;
      }
      const t = new Date(s.created).getTime();
      if (!best || t > new Date(best.created).getTime()) {
        best = s;
      }
    }
    return best;
  ...
  async startClaude(wsId, options) {
    ...
    if (session.superseded) {
      const newest = this.findNewestNonSupersededByAgentAndWd(session.agentName, session.workingDir);
      if (newest && newest.id !== session.id) {
        if (this.dev) {
          console.warn(`[startClaude] sessão obsoleta ${session.id} → ${newest.id}`);
        }
        await this.leaveClaudeSession(wsId);
        await this.joinClaudeSession(wsId, newest.id);
        await this.startClaude(wsId, options);
        return;
      }
      this.sendToWebSocket(wsInfo.ws, {
        type: 'error',
        message: 'Esta sessão expirou. Abre o terminal do agente novamente.',
      });
      return;
    }
