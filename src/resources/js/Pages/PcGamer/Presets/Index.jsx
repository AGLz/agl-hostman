        ...
        <AuthenticatedLayout
            ...
            header={
                <div className="flex items-center justify-between gap-4">
                    <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-200">Presets AMD AM5</h2>
                    ...
                    <Card key={preset.slug}>
                        <CardHeader>
                            <div className="flex items-start justify-between gap-2">
                                ...
                                <Badge variant="outline">{TIER_LABELS[preset.tier] ?? preset.tier}</Badge>
                            </div>
                        ...
                        <CardContent>
                            <p className="mb-4 text-2xl font-semibold">
                                ...
                                <span className="ml-2 text-sm font-normal text-muted-foreground">ref.</span>
                            </p>
                            <ul className="space-y-1 text-sm text-muted-foreground">
                                {(preset.items_json ?? []).slice(0, 5).map((item, i) => (
                                    <li key={i} className="flex justify-between gap-2">
                                        <span className="truncate">{item.label}</span>
                                        <span className="shrink-0">{formatCents(item.reference_cents)}</span>
                                    </li>
                                ))}
                                {(preset.items_json?.length ?? 0) > 5 && (
                                    <li className="text-xs">+ {preset.items_json.length - 5} itens…</li>
                                )}
                            </ul>
