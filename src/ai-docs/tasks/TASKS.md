...
- [ ] **TASK-MC-F2-HERMES-OPENCLAW**: Mission Control Fase 2 — Hermes multi-instância + OpenClaw MC dedicado
...
- [x] **TASK-LLM-MONITOR-F2-DEPLOY**: CT134 migrate + Horizon `llm-monitor` + nginx/php-fpm + cutover `ah.aglz.io` → `192.168.0.134:80` (tunnel **aglsrv1** `f7ab6239`, não archon) — 2026-06-27
- [x] **TASK-LLM-MONITOR-ARGUS-INGEST**: CT188 ingest Argus→CT134 (cron 15m, volume `/var/log/hostman`, smoke `records_written=7`) — 2026-06-27

- [x] **TASK-LLM-MONITOR-F2**: Laravel migrations + `LlmMonitorService` + `LiteLLMClient` + API + Jobs Horizon — repo (2026-06-27)
- [x] **TASK-LLM-MONITOR-DEPLOY-ARGUS**: CT188 — bot `@hermes_jarvis_h_argus_bot` + Telegram connected (2026-06-27)

- [x] **TASK-LLM-MONITOR-F1**: Argus (7º Hermes) + skill `agl-llm-monitor` + CLI `scripts/agl/llm-monitor.sh` + plano + wiki `[[Monitor de Providers LLM AGL]]` + testes Node (2026-06-27)
...
- [x] **TASK-INFRA-OLLAMA-CLOUD-LITELLM-CT200**: _(histórico 2026-05-09 — revertido)_ CT200 chegou a ter 7 modelos Ollama Cloud + overrides alargados; consolidado em **TASK-INFRA-OLLAMA-CT200-QWEN3-4B-ONLY**.
