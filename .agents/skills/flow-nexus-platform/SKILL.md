...
description: Comprehensive Flow Nexus platform management covering user authentication and session management, E2B sandbox creation and execution for isolated code environments, application marketplace deployment and discovery, credit-based payment system and tier management, coding challenges with leaderboards and achievements, cloud storage operations, real-time event subscriptions, and system utilities for platform administration. Use this skill when registering new users with `mcp__flow-nexus__user_register({ email, password, full_name, username })` or authenticating with `mcp__flow-nexus__user_login({ email, password })`, checking authentication status with `mcp__flow-nexus__auth_status({ detailed: true })` to verify active sessions and token validity, creating isolated E2B sandboxes for secure code execution with `mcp__flow-nexus__sandbox_create({ template: 'node' | 'python' | 'go', tier: 'nano' | 'small' | 'medium' | 'large' })`, executing code in sandboxes with `mcp__flow-nexus__sandbox_execute({ sandbox_id, code, timeout: 30000 })` for safe runtime environments, deploying applications to Flow Nexus marketplace with `mcp__flow-nexus__app_deploy({ name, description, template_id, config: {...} })` for discoverability and sharing, browsing marketplace apps with `mcp__flow-nexus__app_list({ category: 'ai-ml' | 'data' | 'web', search: '...' })` to find templates and tools, managing credit balance and payments with `mcp__flow-nexus__credits_balance()` and `mcp__flow-nexus__credits_purchase({ tier: 'starter' | 'pro' | 'enterprise', amount: 1000 })`, tracking usage and billing with `mcp__flow-nexus__credits_history({ period: '30d' })` showing credit consumption by service, participating in coding challenges with `mcp__flow-nexus__challenge_submit({ challenge_id, solution_code, language })` for gamification, viewing leaderboards and achievements with `mcp__flow-nexus__challenge_leaderboard({ challenge_id, limit: 50 })` for competitive programming, uploading files to cloud storage with `mcp__flow-nexus__storage_upload({ path, content, metadata: {...} })` for persistent data, listing and managing stored files with `mcp__flow-nexus__storage_list({ prefix: '/uploads/', limit: 100 })`, subscribing to real-time events with `mcp__flow-nexus__realtime_subscribe({ channel: 'swarm-updates' | 'job-status', callback: handleEvent })` for live monitoring, managing user profiles and preferences with `mcp__flow-nexus__user_profile({ update: {...} })`, administering platform resources with system utility tools for monitoring health, performance metrics, and resource quotas, or troubleshooting authentication failures, sandbox timeout errors, insufficient credits, storage quota limits, or real-time connection issues through platform diagnostics. Essential for managing complete Flow Nexus platform lifecycle from user onboarding to production deployment, executing untrusted code safely in isolated sandbox environments, building and distributing applications through the marketplace ecosystem, implementing usage-based billing with transparent credit tracking, gamifying development workflows with challenges and achievements, storing and retrieving data in cloud infrastructure, monitoring real-time events and system state changes, and maintaining secure, scalable cloud platform operations for AI agent development and deployment.
category: platform
version: 1.0.0
author: Flow Nexus
