...
description: Deploy and manage cloud-based AI agent swarms with event-driven workflow automation, message queue processing, and intelligent agent coordination using Flow Nexus platform supporting multi-topology architectures (hierarchical, mesh, ring, star), dynamic agent spawning and scaling, vector similarity-based agent assignment for optimal task matching, real-time monitoring with comprehensive metrics and audit trails, template library with pre-built swarm configurations, and workflow orchestration with async execution patterns. Use this skill when initializing cloud swarms with `mcp__flow-nexus__swarm_init({ topology: 'hierarchical' | 'mesh' | 'ring' | 'star', maxAgents: 8, strategy: 'balanced' | 'specialized' | 'adaptive' })` for distributed agent coordination, spawning specialized agents dynamically with `mcp__flow-nexus__swarm_spawn({ type: 'coder' | 'reviewer' | 'tester', capabilities: [...], resources: {...} })` based on workload requirements, creating event-driven workflows that process messages from queues with `mcp__flow-nexus__workflow_create({ trigger: 'message_queue', actions: [...], conditions: {...} })` for automated task execution, orchestrating complex multi-step tasks across agent swarms with `mcp__flow-nexus__swarm_orchestrate({ task_id, agents: [...], dependencies: {...} })` handling parallel and sequential execution, using intelligent agent assignment via vector similarity matching between task requirements and agent capabilities with `mcp__flow-nexus__swarm_assign({ task_embedding: [...], agent_pool: [...] })`, monitoring swarm health and performance with `mcp__flow-nexus__swarm_status({ swarm_id, include_metrics: true })` showing agent utilization, task queues, success rates, and bottlenecks, deploying pre-built swarm templates from marketplace with `mcp__flow-nexus__template_deploy({ template_id: 'code-review-swarm' | 'testing-pipeline' | 'data-processing' })` for common orchestration patterns, scaling swarms dynamically with `mcp__flow-nexus__swarm_scale({ swarm_id, target_agents: 12, auto_scale: true })` based on workload or performance metrics, implementing workflow automation with message queue triggers, webhooks, scheduled cron jobs, or custom events using `mcp__flow-nexus__workflow_trigger()`, configuring communication patterns between agents (broadcast, unicast, multicast) and coordination strategies (consensus, leader election, distributed decision making), managing swarm lifecycle with start, pause, resume, and graceful shutdown operations via `mcp__flow-nexus__swarm_control()`, integrating with external systems through webhooks, REST APIs, or message brokers for hybrid cloud-local orchestration, tracking execution history and audit trails with `mcp__flow-nexus__swarm_audit({ swarm_id, time_range: '24h' })` for debugging and compliance, or troubleshooting coordination failures, agent deadlocks, message queue backlogs, or resource contention through Flow Nexus dashboard and logs. Essential for building scalable multi-agent systems without infrastructure management, orchestrating complex workflows with parallel agent execution, implementing event-driven architectures with reliable message processing, coordinating distributed tasks across cloud-based agent networks, automating repetitive development workflows (code review, testing, deployment), and leveraging intelligent agent assignment for optimal resource utilization and task completion. Requires Flow Nexus authentication and active account registration at flow-nexus.ruv.io.
