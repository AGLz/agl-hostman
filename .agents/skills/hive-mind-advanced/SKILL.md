...
description: Advanced Hive Mind collective intelligence system for queen-led multi-agent coordination implementing strategic queen agents orchestrating high-level objectives, specialized worker agents (researcher, coder, analyst, tester, architect, reviewer, optimizer, documenter), Byzantine consensus mechanisms (majority, weighted, unanimous), and persistent collective memory with SQLite persistence, LRU caching, and memory consolidation. Use this skill when orchestrating complex multi-agent systems with `npx Codex-flow swarm init --topology hierarchical --pattern hive-mind` for queen-led architecture, spawning specialized worker agents with `npx Codex-flow agent spawn --type researcher|coder|tester --role worker` for task execution, implementing consensus decisions with majority voting (simple democratic), weighted voting (queen vote 3x weight for strategic guidance), or unanimous voting (critical decisions requiring full agreement), managing collective memory with shared knowledge base using `mcp__claude-flow__memory_usage({ namespace: "hive-mind", key: "collective/knowledge" })` for cross-agent knowledge sharing, coordinating strategic queens for high-level planning, tactical queens for mid-level execution, or adaptive queens that dynamically adjust strategies based on performance metrics, leveraging worker specialization with researchers for analysis/investigation, coders for implementation/development, analysts for data processing/metrics, testers for quality assurance/validation, architects for system design/planning, reviewers for code review/improvement, optimizers for performance enhancement, and documenters for documentation generation, implementing memory persistence with SQLite in WAL mode, LRU cache with memory pressure handling, automatic consolidation removing duplicates, and access pattern tracking for optimization, using consensus mechanisms to coordinate distributed decision-making across agent swarms, synchronizing collective knowledge through persistent memory shared by all agents in the hive, adapting queen strategies dynamically based on worker performance, task complexity, and environmental changes, managing memory consolidation with automatic duplicate removal and associationbuilding between related knowledge, or troubleshooting consensus deadlocks, memory synchronization issues, worker coordination failures, or strategic planning bottlenecks. Essential for building sophisticated multi-agent systems with clear leadership hierarchy, coordinating distributed AI workflows with democratic or weighted decision-making, implementing collective intelligence where knowledge is shared across all agents, scaling complex tasks through specialized worker agents with distinct capabilities, maintaining persistent memory across agent lifecycles and swarm restarts, and achieving optimal performance through strategic queens that adapt coordination patterns based on real-time metrics.
version: 1.0.0
...
The Hive Mind system represents the pinnacle of multi-agent coordination in Codex Flow, implementing a queen-led hierarchical architecture where a strategic queen coordinator directs specialized worker agents through collective decision-making and shared memory.
...
npx Codex-flow hive-mind init --config hive-config.json
```
...
npx Codex-flow hive-mind spawn "Build microservices architecture"

# Strategic queen type
npx Codex-flow hive-mind spawn "Research AI patterns" --queen-type strategic

# Tactical queen with max workers
npx Codex-flow hive-mind spawn "Implement API" --queen-type tactical --max-workers 12

# Adaptive queen with consensus
npx Codex-flow hive-mind spawn "Optimize system" --queen-type adaptive --consensus byzantine

# Generate Codex commands
npx Codex-flow hive-mind spawn "Build full-stack app" --Codex
```
...
npx Codex-flow hive-mind metrics

# Monitor collective memory
npx Codex-flow hive-mind memory
```
...
npx Codex-flow hive-mind sessions

# Pause a session
npx Codex-flow hive-mind pause <session-id>

# Resume a paused session
npx Codex-flow hive-mind resume <session-id>

# Stop a running session
npx Codex-flow hive-mind stop <session-id>
```
...
Generate Codex spawn commands directly:

```bash
npx Codex-flow hive-mind spawn "Build REST API" --Codex
```
...
npx Codex-flow sparc tdd "User authentication" --hive-mind
...
npx Codex-flow hive-mind spawn "Analyze repo quality" --objective "owner/repo"

# PR review coordination
npx Codex-flow hive-mind spawn "Review PR #123" --queen-type tactical
```
...
Hive Mind integrates with Codex Flow hooks for automation:
...
```bash
npx Codex-flow hive-mind spawn "Research ML frameworks" --queen-type strategic
```

**Tactical Queens** - For implementation and execution
```bash
npx Codex-flow hive-mind spawn "Build authentication" --queen-type tactical
```

**Adaptive Queens** - For optimization and dynamic tasks
```bash
npx Codex-flow hive-mind spawn "Optimize performance" --queen-type adaptive
```
...
npx Codex-flow hive-mind memory --gc

# Optimize database
npx Codex-flow hive-mind memory --optimize

# Export and clear
npx Codex-flow hive-mind memory --export --clear
```
...
npx Codex-flow hive-mind spawn "..." --consensus weighted

# Or use simple majority
npx Codex-flow hive-mind spawn "..." --consensus majority
```
...
Define specialized workers in `.Codex/agents/`:
...
```bash
# Frontend hive
npx Codex-flow hive-mind spawn "Build UI" --name frontend-hive

# Backend hive
npx Codex-flow hive-mind spawn "Build API" --name backend-hive
...
npx Codex-flow hive-mind export <session-id> --output backup.json

# Import session
npx Codex-flow hive-mind import backup.json
```
...
npx Codex-flow hive-mind spawn "Build e-commerce platform" \
  --queen-type strategic \
  --max-workers 10 \
  --consensus weighted \
  --Codex

# Output generates Codex commands:
# - Queen coordinator
...
```bash
# Spawn research hive
npx Codex-flow hive-mind spawn "Research GraphQL vs REST" \
  --queen-type adaptive \
  --consensus byzantine
...
```bash
# Review coordination
npx Codex-flow hive-mind spawn "Review PR #456" \
  --queen-type tactical \
  --max-workers 6
...
- [Hive Mind Documentation](https://github.com/ruvnet/Codex-flow/docs/hive-mind)
- [Collective Intelligence Patterns](https://github.com/ruvnet/Codex-flow/docs/patterns)
- [Byzantine Consensus](https://github.com/ruvnet/Codex-flow/docs/consensus)
- [Memory Optimization](https://github.com/ruvnet/Codex-flow/docs/memory)
