...
description: "Master advanced AgentDB features including QUIC synchronization with <1ms latency for real-time distributed systems, multi-database management for agent coordination across multiple knowledge bases, custom distance metrics (cosine, euclidean, dot product, or user-defined functions), hybrid search combining vector similarity with metadata filtering, MMR (Maximal Marginal Relevance) for diverse result sets, and distributed systems integration patterns. Use this skill when implementing QUIC-based synchronization between distributed agents using `agentdb.sync.quic.startServer()` and `agentdb.sync.quic.startClient()` for sub-millisecond inter-node communication, managing multiple specialized databases for different agent capabilities or knowledge domains with `createAgentDBAdapter({ databases: {...} })`, creating custom distance metrics for domain-specific similarity comparisons (e.g., time-series similarity, graph distance, semantic relevance with contextual weights) beyond standard cosine/euclidean metrics, implementing hybrid search that filters by agent role, timestamp, category, or custom metadata before vector similarity ranking with `search({ query, filter: {...}, k: 10 })`, using MMR to ensure result diversity in recommendation systems or knowledge retrieval with `mmr: true, lambda: 0.5` parameters to balance relevance vs diversity, building distributed AI systems where agents synchronize knowledge in real-time across network boundaries with automatic retry and recovery, implementing production-ready patterns with connection pooling, retry strategies, circuit breakers, and graceful degradation for fault-tolerant deployments, optimizing performance with environment variables like `AGENTDB_VECTOR_DIMENSIONS`, `AGENTDB_HNSW_M`, `AGENTDB_HNSW_EF_CONSTRUCTION` for HNSW index tuning, configuring QUIC sync with `syncPort: 4433` and `syncPeers: ['192.168.1.10:4433', '192.168.1.11:4433']` for multi-node clusters, or troubleshooting synchronization issues, connection failures, or performance bottlenecks in distributed deployments. Essential for building scalable multi-agent systems with real-time knowledge sharing across geographical boundaries, implementing sophisticated retrieval strategies beyond simple similarity search, customizing AgentDB for specialized domains requiring unique distance calculations, ensuring high availability and fault tolerance in production AI systems with automatic failover, and achieving optimal performance through advanced configuration and architectural patterns for sub-millisecond response times."
---
